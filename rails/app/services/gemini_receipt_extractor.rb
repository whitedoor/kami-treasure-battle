...
class GeminiReceiptExtractor
  ...
  def self.extract_via_api_key!(image_path:, mime_type:)
    ...
    prompt = <<~PROMPT
      ...
      - card.name は **英単語またはカタカナ表記のみ**にする（漢字・ひらがなは絶対に使わない）。
      - card.name は短く力強く（目安: 6〜18文字、英語の場合はスペース除く）。記号や型番/バーコード/品番、店舗名は入れない。
      - card.name の例: "ARCANE NOVA", "SHADOW SIGIL", "NEON ORB", "アークノヴァ", "シャドウシジル"
      - 画像が読めず items が弱くても card.name は必ず空にしない（英単語/カタカナで創作名を必ず付ける）。
